...
@export_range(0.1, 10, 0.001, "suffix:s") var zoom_timer_delay = 0.25
...
func _ready() -> void:
	...
	zoom_timer.wait_time = zoom_timer_delay
...
func _input(event):
	...
	if Input.is_action_pressed("action_reset"):
		_reset_head_rotation()
		zoom_reset()
		

func _process(delta: float) -> void:
	if Input.is_action_pressed("action_look"):
...
func _rotate_head_to_mouse() -> void:
	if not input_direction_changed:
...
func zoom_reset() -> void:
	...
	tween.tween_method(pcam.set_zoom, pcam.get_zoom(), default_zoom, zoom_duration).set_trans(zoom_trans).set_ease(zoom_ease)

func _start_zoom_tween(target_zoom: Vector2) -> void:
	zoom_timer.one_shot = true
	#print("start zoom tween")
	InputManager.mouse.change_cursor_shape(InputManager.mouse.MouseCursorShape.ZOOM)
	var tween: Tween = create_tween()
