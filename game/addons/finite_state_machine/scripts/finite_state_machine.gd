...
var prev_state : StateMachineState = null
...
signal state_changed(prev_state: StateMachineState, new_state: StateMachineState)
...
func set_current_state(next_state: StateMachineState) -> void:
	...
	# Enter the new state
	if is_instance_valid(current_state):
		current_state.client = self.get_parent()
